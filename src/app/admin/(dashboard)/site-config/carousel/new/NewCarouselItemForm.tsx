...
interface NewCarouselItemFormProps {
  photos: PhotoItem[];
  categories: { id: number; name: string }[];
  initial?: InitialCarousel;
}

export default function NewCarouselItemForm({
  photos,
  categories,
  initial,
}: NewCarouselItemFormProps) {
  const isEdit = !!initial;
  const [selectedId, setSelectedId] = useState<number | null>(initial?.photo_id ?? null);
  const [title, setTitle] = useState(initial?.title ?? '');
  const [caption, setCaption] = useState(initial?.caption ?? '');

  const selected = photos.find((p) => p.id === selectedId) ?? null;

  // 展示与提交的照片信息：优先当前选中，编辑模式回退到原记录
  const info = selected
    ? {
  ...
  const action = isEdit ? updateCarouselItemAction : createCarouselItemAction;
  const submitLabel = isEdit ? '保存修改' : '创建轮播项';
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      >
        ...
        <FormField label="说明文字" name="caption" hint="默认取自照片描述，可修改">
          <Textarea
            name="caption"
            value={caption}
